...
import { getPasswordChar, findByValue, getAsVal, strCorNum } from './utils';
import { DIC_SPLIT, DIC_SHOW_SPLIT, DATE_LIST } from './variable';
...
export const detail = (row = {}, column = {}, option = {}, dic = []) => {
  ...
  if (['array'].includes(type)) {
    ...
    result = `<i class="ac-crud__color" style="background-color:${result}"></i>`;
  } else if (['icon'].includes(type)) {
    result = `<i class="ac-crud__icon ${result}" ></i>`;
  }
  // 日期处理
