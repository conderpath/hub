...
export default {
  ...
  props: {
    ...
    id: {
      type: String,
      default: () => {
        return 'ac-ueditor-' + Math.ceil(Math.random() * 100)
      },
    },
    disabled: {
      type: Boolean,
  ...
  methods: {
    initEdit () {
      ...
      if(this.editor.config.callback) {
        this.editor.config.callback && this.editor.config.callback(this.editor,E)
      }
      this.editor.create()
      this.initPlugins()
    ...
    initUploadImg () {
      if (this.customConfig.uploadImgShowBase64) return
