  ...
  <div class="component-transfer">
    ...
    <div class="transfer-left">
      ...
      <div class="transfer-main">
        <slot name="from"></slot>
        <el-input v-if="filter" clearable size="small" :placeholder="placeholder" v-model="filterFrom" class="filter-tree"></el-input>
        <el-tree
          ref="from-tree"
          show-checkbox
          :lazy="lazy"
...
import {findParents, validatenull} from "../../core/utils/utils";
...
export default {
  ...
  methods: {
    ...
    authorizationAutoCheckRight(nodeObj) {
      const parents = findParents(nodeObj, this.to_array_clone, {
        id: this.node_key,
        parentId: this.pid,
        // children: this.selfDefaultProps.children,
        root: this.rootPidValue,
...
/*      // 查询所有子节点
      ...
      // 过滤掉已经选中过的子节点
      const autoAddParents = differenceBy(parents, this.to_check_keys, this.node_key);
      autoAddParents.forEach((i) => {
