<template>
  <div :class="b()"
    ...
    <el-form :model="form"
      ...
      <el-form-item :label="tenantType.label || '是否租户'"
                    :rules="tenantType.rules"
                    :label-width="setPx(tenantType.labelWidth)"
                    v-if="!tenantType.hide"
                    prop="tenantType">
        <el-tooltip :content="tenantType.tip"
                    :disabled="tenantType.tip===undefined"
                    placement="top-start">
          <el-select size="small" v-model="form.tenantType">
            <el-option label="是" :value="0" ></el-option>
            <el-option label="否" :value="1" ></el-option>
          </el-select>
        </el-tooltip>
      </el-form-item>
      <el-form-item :label="tenantCode.label || '租户编码'"
                    :rules="tenantCode.rules"
                    :label-width="setPx(tenantCode.labelWidth)"
                    v-if="!tenantCode.hide"
                    prop="tenantCode">
        <el-tooltip :content="tenantCode.tip"
                    :disabled="tenantCode.tip===undefined"
                    placement="top-start">
          <el-input v-model="form.tenantCode"
                    size="small"
                    @keyup.enter.native="onSubmit"
                    :placeholder="tenantCode.placeholder || '请输入租户编码'"
                    :autocomplete="tenantCode.autocomplete"></el-input>
        </el-tooltip>
      </el-form-item>
...
export default create({
  ...
  computed: {
    ...
    tenantType () {
      return this.column.tenantType || {}
    },
    tenantCode () {
      return this.column.tenantCode || {}
    }
  ...
  watch: {
    'form.tenantType':{
      handler(val){
        this.option.column.tenantCode.rules.required = val == 0
        this.option.column.tenantCode.hide = val == 1
        if(val==1) {
          this.form.tenantCode = ''
        }else {
          this.form.tenantCode = 'FSG'
        }
      },
      immediate:true,
      deep:true
