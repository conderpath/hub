<template>
  <div>
    ...
    <!-- 单选按钮 -->
    <el-table-column v-if="crud.tableOption.singleSelectionRadio && !crud.tableOption.selection"
                     :fixed="vaildData(crud.tableOption.singleSelectionFixed,config.singleSelectionFixed)"
                     :width="crud.tableOption.singleSelectionWidth || config.singleSelectionWidth"
                     align="center">
      <template slot-scope="{row,$index}">
        <el-radio v-model="currentRowData" :label="row">
          {{ '' }}
        </el-radio>
      </template>
    </el-table-column>
