<template>
  <el-collapse-transition>
    <el-card :shadow="crud.isShadow"
             :class="b()"
             v-show="searchShow && searchFlag">
      <ac-form :option="option"
                 ref="form"
                 @submit="searchChange"
                 @change="handleChange"
...
<script>
import cteate from "../../core/utils/create";
import { vaildData } from "../../core/utils/utils";
import { validatenull } from "../../core/utils/utils";
...
export default cteate({
  ...
  computed: {
    isSearchIcon () {
      return this.vaildData(this.crud.option.searchIcon, this.$ACVU.searchIcon) === true && this.columnLen > this.searchIndex
    },
    searchIndex () {
    ...
    option () {
      ...
      const dataDetail = (list) => {
        ...
        result = Object.assign(result, {
          ...
          menuComponent: option.menuSearchComponent || [],
          menuClass:(()=>{
