<template>
  <div :class="b({'card':tableOption.shadow ? false : !tableOption.card,'no-padding': !vaildData(tableOption.dividerShow,true)})">
    ...
    <el-divider v-if="vaildData(tableOption.dividerShow,true) && !tableOption.card && (($refs.headerSearch || {}).searchFlag&&vaildData(tableOption.searchShowBtn,true))" />

    <el-card :shadow="isShadow">
      <!--      自定义头部-->
      <header-table ref="headerTable">
      ...
      <el-form :model="cellForm"
        ...
        <el-table
                  ...
                  :class="{'ac-crud--indeterminate':vaildData(tableOption.indeterminate,false),'row-no-bottom-border': vaildData(tableOption.stripe,true) && !tableOption.border}"
                  :size="$ACVU.tableSize || controlSize"
          ...
          <template slot="empty">
            <div :class="b('empty')">
              ...
              <ac-empty v-else
                          ...
                          :desc="tableOption.emptyText || '暂无数据'"></ac-empty>
            </div>
          </template>
...
import init from "../../core/mixins/init.js";
...
import { calcCascader, formInitVal } from "../../core/utils/dataformat";
export default create({
  ...
  computed: {
    ...
    calcHeight () {
      return (this.tableOption.calcHeight || 0) + this.$ACVU.calcHeight
    },
    propOption () {
      let result = [];
      const safe = this;
...
/*    isCard () {
    ...
    isShadow () {
      return this.option.shadow ? 'always' : 'never'
    },
    isGroup () {
  ...
  methods: {
    ...
    setCurrentRow (row) {
      this.$refs.columnDefault.currentRowData = row
    ...
    currentRowChange (currentRow, oldCurrentRow) {
      this.$refs.columnDefault.currentRowData = currentRow
  ...
  /*    setTimeout(() => {
    ...
    rowEdit (row, index) {
      ...
      this.$refs.dialogForm.show("edit", index, row);
    },
    //复制
    rowCopy (row) {
      this.tableForm = this.rowClone(row);
      ...
      this.tableIndex = -1;
      this.$emit("input", this.tableForm);
      this.$refs.dialogForm.show("add",-1,row);
    },
    //查看
    rowView (row, index) {
      this.tableForm = this.rowClone(row);
      this.tableIndex = index;
      this.$emit("input", this.tableForm);
      this.$refs.dialogForm.show("view",-1,row);
    },
    vaildParent (row) {
...
.ac-crud{
  ...
  /deep/.hover-row{
    td{
      background-color: @background-theme-color !important;
    }
  }
