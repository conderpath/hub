  ...
  <el-drawer :class="[b('dialog'),b('filter')]"
    ...
    <el-row :span="24">
      ...
      <el-col :md="12"
              ...
              :key="index"
              :class="b('filter-item')">
        <ac-select v-model="column.text"
                     :dic="columnOption"
                     :props="columnProps"
                     :clearable="false"
                     @change="handleChange(column.text,index)"
                     :size="crud.isMediumSize"
                     :class="b('filter-label')"></ac-select>
        <ac-select :class="b('filter-symbol')"
                     v-model="column.symbol"
                     :dic="symbolDic"
                     :clearable="false"
                     :size="crud.isMediumSize"></ac-select>
      ...
      <el-col :span="24"
              class="ac-form__menu ac-form__menu--right">
        <el-button type="primary"
                   :size="crud.isMediumSize"
...
<script>
import { getSearchType, formInitVal } from "../../core/utils/dataformat";
import locale from "../../core/mixins/locale";
import create from "../../core/utils/create";
import { dateList } from "../../core/utils/dataformat";
